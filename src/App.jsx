...
async function fetchAll() {
  ...
  // Numérateur : items avec intervention?=true → somme prix_intervention
  // Dénominateur : tous les items → somme Total_HT
  const numByProject   = {};
...
const S_COLOR = {
  "Saisie d'information":"#64748b","Chiffrage en cours":"#06b6d4","Validé par l'administration":"#8b5cf6",
  "Devis envoyé":"#3b82f6","Devis signé":"#22c55e","Projet terminé":"#10b981",
  "A relancer":"#f59e0b","Relance envoyée":"#f97316","Classé sans suite":"#ef4444","Non formalisé":"#6b7280",
  "Planifié":"#a78bfa","En cours":"#3b82f6","Terminé":"#22c55e","Annulé":"#ef4444",
};
...
function Badge({ label }) {
  ...
  return <span style={{ fontSize:9, fontWeight:700, letterSpacing:"0.05em", textTransform:"uppercase", padding:"3px 6px", borderRadius:3, color:c, background:`${c}18`, border:`1px solid ${c}44`, whiteSpace:"nowrap", display:"inline-block", maxWidth:"100%", overflow:"hidden", textOverflow:"ellipsis" }}>{label}</span>;
}
...
function MultiSelect({ label, options, selected, onChange, colorMap }) {
  ...
  return (
    <div ref={ref} style={{ position:"relative" }}>
      <button onClick={() => setOpen(o=>!o)} style={{ cursor:"pointer", display:"flex", alignItems:"center", gap:5, padding:"5px 9px", borderRadius:4, border:`1px solid ${active?"#3b82f644":"#1e293b"}`, background:active?"rgba(59,130,246,0.08)":"#0a0e14", color:active?"#3b82f6":"#475569", fontSize:10, fontWeight:700, letterSpacing:"0.07em", textTransform:"uppercase" }}>
      ...
      {open && (
        <div style={{ position:"absolute", top:"calc(100% + 4px)", left:0, zIndex:200, background:"#0f1722", border:"1px solid #1e293b", borderRadius:6, padding:6, minWidth:200, maxHeight:280, overflowY:"auto", boxShadow:"0 8px 24px rgba(0,0,0,0.6)" }}>
          ...
          {options.map(opt => {
            const c = colorMap?.[opt]||"#94a3b8"; const sel = selected.includes(opt);
            return (
...
function DateRange({ dateFrom, dateTo, onChange, label }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:5, padding:"4px 8px", borderRadius:4, border:`1px solid ${active?"#3b82f644":"#1e293b"}`, background:active?"rgba(59,130,246,0.06)":"transparent" }}>
      {label && <span style={{ fontSize:10, color:active?"#3b82f6":"#64748b", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.07em", marginRight:2 }}>{label}</span>}
...
function ProgressBar({ value }) {
  ...
  const color = pct >= 80 ? "#22c55e" : pct >= 50 ? "#f59e0b" : pct >= 20 ? "#3b82f6" : "#475569";
  return (
    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
      <div style={{ flex:1, height:4, background:"#1e293b", borderRadius:2 }}>
        <div style={{ height:4, background:color, width:`${pct}%`, borderRadius:2, transition:"width 0.3s" }} />
      </div>
      <span style={{ fontSize:10, color:color, fontWeight:700, width:30, textAlign:"right", flexShrink:0 }}>{pct}%</span>
    </div>
  );
}
...
function TabDevis({ offers, selectedCompany, onSelectCompany }) {
  // Filtre période global (KPIs + graphiques)
  ...
  // Top clients sur offresInPeriod
  const byClient={};
  active.forEach(o=>{
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16, padding:"10px 14px", background:"#0f1722", border:"1px solid #1e293b", borderRadius:6 }}>
        <span style={{ fontSize:10, color:"#475569", letterSpacing:"0.1em", textTransform:"uppercase", fontWeight:700 }}>Période KPIs</span>
        <DateRange dateFrom={periodFrom} dateTo={periodTo} onChange={(f,t)=>{setPeriodFrom(f);setPeriodTo(t);}} />
        {hasPeriod && <span style={{ fontSize:10, color:"#64748b" }}>{offersInPeriod.length} devis dans la période</span>}
        {!hasPeriod && <span style={{ fontSize:10, color:"#334155" }}>Toutes les périodes</span>}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 340px", gap:12, marginBottom:20 }}>
        <div style={{ background:"#0f1722", border:"1px solid #1e293b", borderRadius:6, padding:20 }}>
          <div style={{ fontSize:10, color:"#475569", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:14 }}>
            Répartition CA par statut {hasPeriod&&<span style={{ color:"#3b82f6", fontSize:9 }}>· période filtrée</span>}
          </div>
        ...
        <div style={{ background:"#0f1722", border:"1px solid #1e293b", borderRadius:6, padding:20 }}>
          <div style={{ fontSize:10, color:"#475569", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:14 }}>
            Top clients {hasPeriod&&<span style={{ color:"#3b82f6", fontSize:9 }}>· période filtrée</span>}
          </div>
          {topClients.map((c,i)=>(
      ...
      <div style={{ background:"#0f1722", border:"1px solid #1e293b", borderRadius:6, padding:20 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"150px 1fr 150px 150px 100px 120px 70px", gap:8, padding:"7px 10px", borderBottom:"1px solid #1e293b", fontSize:9, letterSpacing:"0.1em", textTransform:"uppercase" }}>
        ...
        <div style={{ maxHeight:520, overflowY:"auto" }}>
          {filtered.length===0
            ...
            : filtered.map(o=>{
              ...
              return (
                <div key={o.id} style={{ display:"grid", gridTemplateColumns:"150px 1fr 150px 150px 100px 120px 70px", gap:8, padding:"9px 10px", borderBottom:"1px solid #0a1120", alignItems:"center" }}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"150px 1fr 150px 150px 100px 120px 70px", gap:8, padding:"10px 10px 4px", borderTop:"1px solid #1e293b" }}>
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16, padding:"10px 14px", background:"#0f1722", border:"1px solid #1e293b", borderRadius:6 }}>
        <span style={{ fontSize:10, color:"#475569", letterSpacing:"0.1em", textTransform:"uppercase", fontWeight:700 }}>Période KPIs</span>
        <DateRange dateFrom={periodFrom} dateTo={periodTo} onChange={(f,t)=>{setPeriodFrom(f);setPeriodTo(t);}} />
        {hasPeriod && <span style={{ fontSize:10, color:"#64748b" }}>{intervInPeriod.length} interventions dans la période</span>}
        {!hasPeriod && <span style={{ fontSize:10, color:"#334155" }}>Toutes les périodes</span>}
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
        <KpiCard label="Total"      value={intervInPeriod.length} sub={`sur ${projects.length} projets`} accent="#3b82f6" pct={100} />
        <KpiCard label="Terminées"  value={terminees.length}  sub={`${intervInPeriod.length?Math.round((terminees.length/intervInPeriod.length)*100):0}% du total`} accent="#22c55e" pct={intervInPeriod.length?(terminees.length/intervInPeriod.length)*100:0} />
        ...
        <KpiCard label="Planifiées" value={planifiees.length} sub="à venir"  accent="#a78bfa" pct={intervInPeriod.length?(planifiees.length/intervInPeriod.length)*100:0} />
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 280px", gap:12, marginBottom:20 }}>
        <div style={{ background:"#0f1722", border:"1px solid #1e293b", borderRadius:6, padding:20 }}>
          <div style={{ fontSize:10, color:"#475569", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:14 }}>
            Types de prestations {hasPeriod&&<span style={{ color:"#3b82f6", fontSize:9 }}>· période filtrée</span>}
          </div>
        ...
        <div style={{ background:"#0f1722", border:"1px solid #1e293b", borderRadius:6, padding:20 }}>
          <div style={{ fontSize:10, color:"#475569", letterSpacing:"0.12em", textTransform:"uppercase", marginBottom:14 }}>
            Activité mensuelle {hasPeriod&&<span style={{ color:"#3b82f6", fontSize:9 }}>· période filtrée</span>}
          </div>
